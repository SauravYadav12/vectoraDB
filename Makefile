...
.PHONY: build vet fmt vm-build test integration web-dev web-build release wsl-kernel
...
release:          ## Cross-compile release binaries into ./dist (darwin+linux+windows)
	@rm -rf dist && mkdir -p dist
	@for t in darwin/arm64 darwin/amd64 linux/arm64 linux/amd64 windows/amd64; do \
		os=$${t%/*}; arch=$${t#*/}; ext=""; [ "$$os" = "windows" ] && ext=".exe"; \
		echo "  building vdb-$$os-$$arch$$ext"; \
		CGO_ENABLED=0 GOOS=$$os GOARCH=$$arch \
			go build -trimpath -ldflags "$(LDFLAGS)" -o dist/vdb-$$os-$$arch$$ext ./cmd/vdb; \
	done
	@echo "release binaries in ./dist (version $(VERSION))"
	@echo "note: the Windows installer also needs the ZFS-enabled WSL2 kernel + Ubuntu rootfs assets (see make wsl-kernel / docs/windows-setup.md)"
...
wsl-kernel:       ## Build the ZFS-enabled WSL2 kernel + Ubuntu rootfs (Linux builder/CI only)
	deploy/wsl-kernel/build.sh
	deploy/wsl-kernel/build-rootfs.sh
